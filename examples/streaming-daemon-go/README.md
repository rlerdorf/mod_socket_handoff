...
- **`gc_percent`** - Sets `GOGC` via `debug.SetGCPercent()`. Controls how much the heap can grow before triggering GC. Default is 100 (GC when heap doubles). Set higher (e.g., 200) when using `memlimit` to reduce GC frequency and let the memory limit drive collection instead.
